...
class QuestionScopeResolver:
    ...
    def _question_references_previous_scope(cls, question: str) -> bool:
        ...
        if tokens & reference_tokens and tokens & scope_nouns:
            return True
        if re.search(
            r"\b(?:estos|estas|esos|esas)\s+\d+\s+(?:sitios|contratos|archivos|documentos|folios|casos)\b",
            normalized,
        ):
            return True
        if re.search(
            r"\b(?:este|esta|ese|esa)\s+(?:(?:mismo|misma|anterior|previo|previa|mencionado|mencionada|listado|listada|devuelto|devuelta)\s+)?(?:sitio|contrato|archivo|documento|folio|caso|resultado|zip)\b",
            normalized,
        ):
            return True
